...
<script>
    ...
    function navigateToCandidate(candidateId) {
        if (race) {
            saveSourceRace('congress', race['race-id']);
            goto(`${base}/congress/candidate/${candidateId}`);
        }
    }
    ...
    async function loadRaceData() {
        try {
            ...
            race = await getRaceByRaceId(raceId, 'US Congress');
            ...
            for (let i = 1; i <= 5; i++) {
                ...
                if (race[candidateKey]) {
                    const candidate = await getCandidateByCandidateId(race[candidateKey], 'US Congress');
                    if (candidate) {
