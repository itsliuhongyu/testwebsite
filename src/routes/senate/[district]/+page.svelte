...
<script>
    ...
    function navigateToCandidate(candidateId) {
        if (race) {
            saveSourceRace('senate', race['race-id']);
            goto(`${base}/senate/candidate/${candidateId}`);
        }
    }
    ...
    async function loadRaceData() {
        try {
            ...
            race = await getRaceByRaceId(raceId, 'Senate');
            ...
            for (let i = 1; i <= 5; i++) {
                ...
                if (race[candidateKey]) {
                    const candidate = await getCandidateByCandidateId(race[candidateKey], 'Senate');
                    if (candidate) {
