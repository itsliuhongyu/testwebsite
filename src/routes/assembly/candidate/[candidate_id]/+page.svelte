...
<script>
    ...
    function returnToRace() {
        ...
        if (sourceRace && sourceRace.raceType === 'assembly') {
            clearSourceRace();
            goto(`${base}/assembly/${sourceRace.raceId}`);
        } else if (raceId) {
            clearSourceRace();
            goto(`${base}/assembly/${raceId}`);
        } else {
            clearSourceRace();
            goto(`${base}/`);
        }
    ...
    async function fetchCandidate() {
        try {
            ...
            candidate = await getCandidateByCandidateId(candidateId, 'Assembly');
            
            // Find the race that contains this candidate
            if (candidateId) {
                const races = await fetchRacesFromAPI('Assembly');
