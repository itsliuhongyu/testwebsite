...
<script>
    ...
    function returnToRace() {
        ...
        if (sourceRace && sourceRace.raceType === 'senate') {
            clearSourceRace();
            goto(`${base}/senate/${sourceRace.raceId}`);
        } else if (raceId) {
            clearSourceRace();
            goto(`${base}/senate/${raceId}`);
        } else {
            clearSourceRace();
            goto(`${base}/`);
        }
    ...
    async function fetchCandidate() {
        try {
            ...
            candidate = await getCandidateByCandidateId(candidateId, 'Senate');
            
            // Find the race that contains this candidate
            if (candidateId) {
                const races = await fetchRacesFromAPI('Senate');
                console.log('All races:', races);
                console.log('Looking for race with candidate-1 or candidate-2 =', candidateId);
                
                const race = races.find(r => {
                    console.log(`Checking race ${r['race-id']}: candidate-1=${r['candidate-1']}, candidate-2=${r['candidate-2']}`);
                    return r['candidate-1'] === candidateId || r['candidate-2'] === candidateId;
                });
                
                console.log('Found race:', race);
                if (race) {
                    raceId = race['race-id'];
                    console.log('Set raceId to:', raceId);
                }
            }
